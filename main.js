...
function create_n_Points(n) {
  ...
  // For n <= 4, regular polygon is fine (no triple intersections)
  if (n <= 4) {
    for (let i = 0; i < n; ++i) {
      theta = i * 2 * Math.PI / n;
    ...
    // For n > 4, we need to perturb the points to avoid triple intersections
    // Strategy: Use slightly unequal spacing around the circle
    
    // First, determine if we need perturbation
    // Regular polygon has triple intersections when n is even
    const needsPerturbation = (n % 2 === 0);
    
    if (!needsPerturbation) {
      // For odd n, regular polygon works fine
      for (let i = 0; i < n; ++i) {
        theta = i * 2 * Math.PI / n;
        pts.push([center[0] + r * Math.cos(theta), center[1] + r * Math.sin(theta)]);
      }
    } else {
      // For even n, perturb the angles more significantly
      // We want to keep points roughly equally spaced but break symmetry
      
      // Start with regular spacing
      const baseAngles = [];
      for (let i = 0; i < n; ++i) {
        baseAngles.push(i * 2 * Math.PI / n);
      }
      
      // Use larger perturbations to ensure intersections are well separated
      // The perturbation amount should scale with n to maintain separation
      const basePerturb = 0.04 + 0.01 * Math.sqrt(n / 6);
      
      let bestPerturbation = null;
      let bestMinDistance = 0;
      let attempts = 0;
      const maxAttempts = 100;
      
      // Try different perturbation patterns to find one that gives good separation
      while (attempts < maxAttempts) {
        attempts++;
        
        // Generate perturbations using multiple sine/cosine components
        const perturbedAngles = baseAngles.map((angle, i) => {
          // Use multiple frequency components for more varied perturbations
          const perturb = basePerturb * (
            0.5 * Math.sin(i * 1.7 + 0.5 * attempts) +
            0.3 * Math.cos(i * 2.3 + 1.3 * attempts) +
            0.2 * Math.sin(i * 3.1 + 0.7 * attempts) +
            0.1 * Math.cos(i * 4.7 + 2.1 * attempts)
          );
          return angle + perturb;
        });
        
        // Sort angles to maintain circular order
        perturbedAngles.sort((a, b) => a - b);
        
        // Create temporary points
        const tempPts = [];
        for (let i = 0; i < n; ++i) {
          const angle = perturbedAngles[i];
          tempPts.push([center[0] + r * Math.cos(angle), center[1] + r * Math.sin(angle)]);
        }
        ...
        for (let i = 0; i < n; i++) {
          for (let j = i + 1; j < n; j++) {
            if (dist(tempPts[i][0], tempPts[i][1], tempPts[j][0], tempPts[j][1]) < eps) {
              tooClose = true;
              break;
            }
          }
        ...
        // Calculate intersections for this perturbation
        const tempSegments = [];
        for (let i = 0; i < tempPts.length - 1; ++i) {
          for (let j = i + 1; j < tempPts.length; ++j) {
            tempSegments.push([tempPts[i][0], tempPts[i][1], tempPts[j][0], tempPts[j][1]]);
          }
        }
        
        const tempIntersections = [];
        for (let i = 0; i < tempSegments.length; i++) {
          for (let j = i + 1; j < tempSegments.length; j++) {
            const inter = findIntersectionPoints(tempSegments[i], tempSegments[j]);
            if (inter) {
              let duplicate = false;
              for (let k = 0; k < tempIntersections.length; k++) {
                if (dist(tempIntersections[k][0], tempIntersections[k][1], inter[0], inter[1]) < 0.1) {
                  duplicate = true;
                  break;
                }
              }
              if (!duplicate) {
                tempIntersections.push(inter);
              }
            }
          }
        }
        
        // Check minimum distance between intersections
        let minDist = Infinity;
        for (let i = 0; i < tempIntersections.length; i++) {
          for (let j = i + 1; j < tempIntersections.length; j++) {
            const d = dist(tempIntersections[i][0], tempIntersections[i][1], 
                          tempIntersections[j][0], tempIntersections[j][1]);
            if (d < minDist) minDist = d;
          }
        }
        
        // If this perturbation gives better separation, keep it
        if (minDist > bestMinDistance) {
          bestMinDistance = minDist;
          bestPerturbation = tempPts;
        }
        
        // If we found good separation (>= 5 pixels), stop searching
        if (minDist >= 10) {
          break;
        }
      }
      
      // Use the best perturbation found
      if (bestPerturbation) {
        pts = bestPerturbation;
      } else {
        // Fallback: use a simpler perturbation
        console.log("Using fallback perturbation");
        const perturbedAngles = baseAngles.map((angle, i) => {
          return angle + 0.05 * (i % 2 === 0 ? 1 : -1) * (1 + 0.2 * (i / n));
        });
        perturbedAngles.sort((a, b) => a - b);
        for (let i = 0; i < n; ++i) {
          const angle = perturbedAngles[i];
          pts.push([center[0] + r * Math.cos(angle), center[1] + r * Math.sin(angle)]);
        }
      }
    }
  }
...
// Helper function to find intersections without using global variables
function findIntersectionPoints(s1, s2) {
...
function colorRegions() {
  const colors = [
    [255, 0, 0],    // Red  [255, 80, 80]
    [0, 255, 0],    // Green [80, 255, 80]
    [0, 0, 255],    // Blue
    [255, 255, 0],   // Yellow
    [255, 0, 255],   // Magenta
    [0, 255, 255]    // Cyan
  ...
  for (let idx of sortedIndices) {
    ...
    const colorUsage = new Array(colors.length).fill(180);
